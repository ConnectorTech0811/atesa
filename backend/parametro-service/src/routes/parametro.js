...
router.patch('/parametro/empresas/:id/representante', async (req, res) => {
  const usuario = verificarAcesso(req, res);
  if (!usuario) return;
  const { representante } = req.body ?? {};
  try {
    await alterarRepresentanteEmpresa(req.params.id, representante, usuario.id, usuario.nome);
    res.json({ ok: true, representante });
  } catch (e) {
    console.error(e);
    res.status(500).json({ erro: 'Erro ao alterar representante.' });
  }
});
