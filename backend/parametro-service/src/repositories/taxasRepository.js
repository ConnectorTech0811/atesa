...
import { CBO_PADRAO, obterCboPorCargo } from './parametroRepository.js';
...
export async function listarCargosPorCooperativa(cooperativa) {
  await garantirTabelasTaxas();
  const [rows] = await pool.query(
    `SELECT id, cargo, cbo, ordem FROM cargos_referencia
     WHERE cooperativa = ? ORDER BY ordem ASC, cargo ASC`,
    [cooperativa]
  );
  return rows.map((r) => ({ ...r, cbo: r.cbo || obterCboPorCargo(r.cargo) }));
}

export async function listarTodasCargos() {
  await garantirTabelasTaxas();
  const [rows] = await pool.query(
    `SELECT id, cooperativa, cargo, cbo, ordem FROM cargos_referencia ORDER BY cooperativa, ordem ASC, cargo ASC`
  );
  // Retorna { COOPERATIVA: [{ id, cargo, cbo, ordem }] }
  const mapa = {};
  for (const r of rows) {
    if (!mapa[r.cooperativa]) mapa[r.cooperativa] = [];
    mapa[r.cooperativa].push({ id: r.id, cargo: r.cargo, cbo: r.cbo || obterCboPorCargo(r.cargo), ordem: r.ordem });
  }
  return mapa;
}
...
export async function substituirCargosDaCooperativa(cooperativa, cargos) {
  await garantirTabelasTaxas();
  // cargos: (string | { cargo: string; cbo?: string })[]
  const conexao = await pool.getConnection();
  try {
    await conexao.beginTransaction();
    await conexao.query(`DELETE FROM cargos_referencia WHERE cooperativa = ?`, [cooperativa]);
    if (cargos.length > 0) {
      const valores = cargos.map((item, i) => {
        const nomeCargo = typeof item === 'string' ? item.trim() : item.cargo.trim();
        const cbo = typeof item === 'object' && item.cbo ? item.cbo : obterCboPorCargo(nomeCargo);
        return [cooperativa, nomeCargo, cbo || null, i + 1];
      });
      await conexao.query(
        `INSERT INTO cargos_referencia (cooperativa, cargo, cbo, ordem) VALUES ?`,
        [valores]
