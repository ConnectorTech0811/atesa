...
const PainelExecutivo: React.FC = () => {
  ...
  return (
    <div className="painel-page">
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '24px 28px', display: 'flex', gap: 24, alignItems: 'flex-start', background: '#fcfdfc' }}>
            ...
            <div style={{ flex: '0 0 460px', maxWidth: 460 }}>
              <div style={{ background: '#ffffff', border: '1px solid #d4e8d5', borderRadius: 14, padding: 22, boxShadow: '0 2px 10px rgba(0,0,0,0.03)' }}>
                ...
                <div style={{ marginBottom: 12 }}>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#555', marginBottom: 4 }}>Destinatário (e-mail) *</label>
                  <input type="email" className="form-input" value={formProposta.destinatario}
                    onChange={(e) => setFormProposta(p => ({ ...p, destinatario: e.target.value }))} placeholder="email@empresa.com" />
                </div>

                <div style={{ marginBottom: 12 }}>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#555', marginBottom: 4 }}>Assunto *</label>
                  <input type="text" className="form-input" value={formProposta.assunto}
                ...
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#555', marginBottom: 4 }}>Observação interna (opcional)</label>
                  <input type="text" className="form-input" value={formProposta.observacao}
                    onChange={(e) => setFormProposta(p => ({ ...p, observacao: e.target.value }))} placeholder="Nota visível só internamente" />
                </div>
            ...
            <div style={{ flex: '1 1 360px', minWidth: 260 }}>
              ...
              {!carregandoPropostas && propostas.length === 0 && (
                <div style={{ background: '#f9f9f9', border: '1px dashed #ccc', borderRadius: 10, padding: '28px 20px', textAlign: 'center', color: '#aaa', fontSize: 13 }}>
                  Nenhuma proposta enviada ainda.<br />
                  <span style={{ fontSize: 12 }}>Preencha o formulário ao lado e clique em Enviar.</span>
                </div>
              )}
              {propostas.map((p) => (
      ...
      {/* ── [removido] Modal principal de ações agora é painel inline (master-detail) ── */}
      {false && <IonModal className="modal-grande" isOpen={false} onDidDismiss={() => {}}>
